...
export class Game {
 constructor(random=Math.random){this.random=random;this.height=H;this.state='ready';this.level=0;this.score=0;this.lives=3;this.quota=0;this.combo=0;this.x=W/2;this.bossX=W/2;this.time=0;this.drops=[];this.particles=[];this.events=[];this.shots=[];this.hitCount=0;this.invincible=0;this.gulp=0;this.release=0;this.windup=0;this.victoryTime=0;this.review=null;this.zones=[];this.timeLeft=42;this.managerX=110;}
 ...
 start(level=0,score=0,lives=3){
  this.level=clamp(level,0,19);this.score=score;this.startScore=score;this.lives=lives;this.quota=0;this.combo=0;this.x=W/2;this.bossX=W/2;this.bossWalk=0;this.walk=0;this.time=0;this.drops=[];this.particles=[];this.shots=[];this.hitCount=0;this.invincible=0;this.gulp=0;this.release=0;this.windup=0;this.pendingDrop=null;this.spawnClock=.85;this.spawnCount=0;this.recoil=0;this.state='playing';this.events=[];this.victoryTime=0;this.zones=[];this.managerX=110;this.timeLeft=LEVELS[this.level].deadline;
  this.review=LEVELS[this.level].review?{...LEVELS[this.level].review,phase:'intro',timer:3.2,introTotal:3.2,warningTime:1.05,rage:false,chained:false,cycle:0,patternNow:'audit',defeatTime:0}:null;
 ...
 spawn(){
  if(this.pendingDrop)return;const c=LEVELS[this.level],r=this.random(),hazard=this.review?.phase==='open'?c.hazard*(this.review.rage?.94:.7):c.hazard,coffee=this.level>=2?.022:0;
  const type=r<hazard?'file':r<hazard+coffee?'coffee':r<hazard+coffee+c.gold?'gold':'poop';
 ...
 releaseThrow(){
  const p=this.pendingDrop;if(!p)return;const drop={x:clamp(this.bossX+28,24,W-24),y:100,vx:p.vx,vy:p.speed*this.flightScale,type:p.type,phase:this.random()*6.28,rotation:(this.random()-.5)*.5};this.spaceHazard(drop);this.drops.push(drop);this.spawnCount++;
 ...
 catchDrop(d){
  if(d.type==='file'){this.damage(d.x,d.y);return;}if(d.type==='coffee'){this.lives=Math.min(3,this.lives+1);this.score+=25;this.burst(d.x,d.y,'#8de4ed');this.event('coffee','COFFEE BREAK +1 ♥');return;}
 ...
 enrage(){const b=this.review;b.rage=true;b.phase='phaseChange';b.timer=1.7;b.open=3.15;b.attack=1.7;b.warningTime=.92;b.chained=false;this.drops=[];this.pendingDrop=null;this.windup=0;this.zones=[];this.event('enrage','FINAL WARNING');}
 updateReview(dt){
  const b=this.review;if(!b)return;this.managerX=105+Math.sin(this.time*1.2)*30;
 ...
 update(dt,input={}){
  ...
  const oldBossX=this.bossX,pace=this.time*(1.2+this.level*.025);if(!this.pendingDrop)this.bossX=this.review&&this.level!==19?350+Math.sin(pace)*104:W/2+Math.sin(pace)*173+Math.sin(pace*2.3+this.level)*27;this.bossWalk=this.bossX-oldBossX;
